...
This document is a step-by-step guide that describes how to install Aïobi Meet on a k8s cluster without AI features.
...
Aïobi Meet uses OIDC, so if you already have an OIDC provider, obtain the necessary information to use it. In the next step, we will see how to configure Django (and thus Aïobi Meet) to use it. If you do not have a provider, we will show you how to deploy a local Keycloak instance (this is not a production deployment, just a demo).
...
In your OIDC provider, set Aïobi Meet's redirect URI to `https://.../api/v1.0/callback/` where `...` should be replaced with the domain name Aïobi Meet is hosted on.
...
Aïobi Meet use livekit for streaming part so if you have a livekit provider, obtain the necessary information to use it. If you do not have a provider, you can install a livekit testing environment as follows:
...
Aïobi Meet uses a postgresql db as backend so if you have a provider, obtain the necessary information to use it. If you do not have, you can install a postgresql testing environment as follows:
...
Now you are ready to deploy Aïobi Meet without AI. AI required more dependencies (Openai-compliant API, LiveKit Egress, Cold storage and a docs deployment to push resumes). To deploy meet you need to provide all previous information to the helm chart.
...
You can use Aïobi Meet on https://meet.127.0.0.1.nip.io from the local device. The provisioning user in keycloak is meet/meet.
...
| FRONTEND_USE_PROCONNECT_BUTTON                  | Show a branded login button in the homepage instead of a generic "Login" button                                                                              | false                                                                                                                                                         |
